...
module.exports = new (function () {
  ...
  if (typeof Float32Array == "undefined") {
    getFloat32Array = getUint8Array = function (len) {
      if (len.length) {
        return len.slice(0);
      }
  ...
  this.convolveVHF = function (
    ...
    var side =
      isVertical && isHorizontal
        ? Math.round(Math.sqrt(weights.length))
        : weights.length;
        ...
        if (isVertical && isHorizontal) {
          for (var cy = 0; cy < side; cy++) {
            for (var cx = 0; cx < side; cx++) {
              var scy = isVertical
                ? Math.min(sh - 1, Math.max(0, sy + cy - halfSide))
                : sy;
              var scx = isHorizontal
                ? Math.min(sw - 1, Math.max(0, sx + cx - halfSide))
                : sx;
  ...
  this.separableConvolve = function (
    pixels,
    horizWeights,
    vertWeights,
    opaque
  ...
  this.separableConvolveFloat32 = function (
    pixels,
    horizWeights,
    vertWeights,
    opaque
  ...
  this.sharpen = function (pixels) {
    return this.convolve(pixels, [0, -1, 0, -1, 5, -1, 0, -1, 0]);
  };
  this.sobel = function (pixels) {
    var px = this.grayscale(pixels);
    var vertical = this.convolveFloat32(px, [-1, -2, -1, 0, 0, 0, 1, 2, 1]);
    var horizontal = this.convolveFloat32(px, [-1, 0, 1, -2, 0, 2, -1, 0, 1]);
  ...
  this.blurC = function (pixels, fact) {
    var w = 1 / 9;
    return this.convolve(pixels, [w, w, w, w, w, w, w, w, w]);
  };
